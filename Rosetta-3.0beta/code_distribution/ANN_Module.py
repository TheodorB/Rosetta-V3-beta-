...
class ANN(object):
    ...
    def parse_transfer_funcs(self,nl,tr):
       ...
       # if tr is None: return([self.tansig,self.purelin])
        ...
        funcs_names=[]
        for i in range(nl):
#            print(trhelp[i].lower().decode("utf-8"))
            if trhelp[i].lower().decode("utf-8")=='tansig': #.decode("utf-8") added by theodor
                funcs.append(self.tansig)
                funcs_names.append('tansig')
            elif trhelp[i].lower().decode("utf-8")=='logsig': #.decode("utf-8") added by theodor
                funcs.append(self.logsig)
                funcs_names.append('logsig')
            elif trhelp[i].lower().decode("utf-8")=='purelin': #.decode("utf-8") added by theodor
                funcs.append(self.purelin)
                funcs_names.append('purelin')
            elif trhelp[i].lower().decode("utf-8")=='none': #.decode("utf-8") added by theodor
                funcs.append(None)
...
class PS(object):
    ...
    def parse_query(self,cursor):
        ...
        for i,row in enumerate(data):
            ...
            d_min[i] = row[11]
            d_max[i] = row[12]
        print("self._var_names : ", self._var_names)
    ...
    def check_data(self,data_in):
        ...
        print("logical_and 1 : ",  N.greater_equal(data_in,self.data_min), "logical_and 2:", N.less_equal(data_in,self.data_max) )
